...
import { getUser, getAllUsers, updateUser, updateMyAdvocacy } from "../handlers/userHandler.js";
...
router.post("/updateMyAdvocacy", async (req: Request, res: Response) => {
  const token = getSessionToken(req);
  const updateData = req.body;
  await updateMyAdvocacy(token, updateData, res);
})
